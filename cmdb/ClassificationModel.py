...
def selectModel(method_name = 'ed_knn'):
    if method_name == 'ed_knn':
...
def execute_data(filename = 'synthetic_control',method = 'ed_knn'):
    results = []
    # trainData = np.loadtxt('./static/UCR_TS_Archive_2015/'+filename+'/'+filename+'_TRAIN',delimiter=',')
    # testData = np.loadtxt('./static/UCR_TS_Archive_2015/'+filename+'/'+filename+'_TEST',delimiter=',')
    # print(trainData)
    trainData = database_op.op_db(filename)[0]
    testData = database_op.op_db(filename)[1]
    ...
    #
    rightRatio1, errorRatio1 = model.evaluate(testData[:, 0], preY1)
    results.append(rightRatio1)
    results.append(errorRatio1)
